...
var generateOutputTransactions = function (poolRecipient, recipients, rpcData, network, poolOptions) {
    ...
    if (!reward) {
        ...
        reward -= rpcData.coinbasetxn.volkshashstaticfee; 
        // TODO Dynamic/Static Processing Fee  reward -= rpcData.coinbasetxn.volkshashfee; 
        /* rpcData.coinbasetxn.fee := <total fee of transaxtions> * -1)
        /*
    ...
    if (rpcData.masternode) {
        if (rpcData.masternode.payee) {
            ...
            var payeeScript = util.kotoAddressToScript(rpcData.masternode.payee);
            ...
            for (var i = 0; i < rpcData.masternode.length; i++) {
                ...
                if (rpcData.masternode[i].script) {
                    ...
                    payeeScript = util.kotoAddressToScript(rpcData.masternode[i].payee);
                }
    ...
    if (rpcData.superblock && rpcData.superblock.length > 0) {
        for (var i = 0; i < rpcData.superblock.length; i++) {
            ...
            if (rpcData.superblock[i].script) {
                ...
                var payeeScript = util.kotoAddressToScript(rpcData.superblock[i].payee);
            }
    ...
    if (rpcData.founderreward) {
        ...
        var payeeScript = util.kotoAddressToScript(rpcData.founderreward.founderpayee);
        txOutputBuffers.push(Buffer.concat([
            util.packInt64LE(payeeReward),
            util.varIntBuffer(payeeScript.length),
            payeeScript
    ...
    if (rpcData.payee) {
        ...
        var payeeScript = util.kotoAddressToScript(rpcData.payee);
        txOutputBuffers.push(Buffer.concat([
            util.packInt64LE(payeeReward),
            util.varIntBuffer(payeeScript.length),
            payeeScript
